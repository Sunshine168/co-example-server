...
qiniu.prototype.init = config => {
  ...
  this.bucket = bucket;
  this.mac = new baseQiniu.auth.digest.Mac(access_key, secret_key);
  this.config = new baseQiniu.conf.Config();
  return this;
};
...
module.exports = {
  ...
  getToken: config => {
    ...
    const putPolicy = new baseQiniu.rs.PutPolicy(options);
    const uploadToken = putPolicy.uploadToken(instance.mac);
    return uploadToken;
  },
  getDownloadUrl: key => {
    const { mac, config } = instance;
    const bucketManager = new baseQiniu.rs.BucketManager(mac, config);
